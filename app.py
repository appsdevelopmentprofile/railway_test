...
This is the document that will host the front end of the demo for the full solution. It starts working as ipynb for fixing (as it has the assistance of Gemini fixer) and demonstration (as it can get the text and results added in sections).

Once it is ready to be run, it will be downloaded as a Python (.py) file and be included into the GitHub repository. It is done in this way so the GitHub is not oversaturated and we are able to use it for multiple deployments.
